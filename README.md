...
Pathfinding Agent (A and GBFS Visualizer)*
...
Select A* or GBFS
Select heuristic (Manhattan or Euclidean)
...
If a wall blocks the remaining path, the agent automatically replans and continues toward the goal.
...
Animation Speed: 80 ms per step
...
Dynamic replanning only triggers if a new obstacle blocks the remaining path.
